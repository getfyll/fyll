-- Fix RLS policies to allow signup (Version 3 - Only existing tables)
...
-- ===================================
-- TEAM_MEMBERS TABLE (if it exists)
-- ===================================
...
BEGIN
  IF EXISTS (
    ...
    -- Drop existing policies
    EXECUTE 'DROP POLICY IF EXISTS team_members_insert_own ON public.team_members';
    EXECUTE 'DROP POLICY IF EXISTS team_members_update_own ON public.team_members';
    EXECUTE 'DROP POLICY IF EXISTS team_members_select_team ON public.team_members';

    -- Allow users to insert themselves as team member (needed for signup)
    EXECUTE 'CREATE POLICY team_members_insert_own
      ON public.team_members FOR INSERT
      WITH CHECK (user_id = auth.uid()::text)';

    -- Allow users to update their own team member record
    EXECUTE 'CREATE POLICY team_members_update_own
      ON public.team_members FOR UPDATE
      USING (user_id = auth.uid()::text)
      WITH CHECK (user_id = auth.uid()::text)';

    -- Allow users to see team members in their business
    EXECUTE 'CREATE POLICY team_members_select_team
    ...
    RAISE NOTICE 'Created team_members policies';
  ELSE
